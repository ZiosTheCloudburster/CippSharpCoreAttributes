    ...
    public class SceneSelectorAttribute : PropertyAttribute
    {
        ...
        public class SceneSelectorPropertyDrawer : PropertyDrawer
        {
            ...
            private static void UpdateSceneNames()
            {
                ...
                tmpSceneNames.AddRange(EditorBuildSettings.scenes.Select(s =>
                        new KeyValuePair<string, bool>(AssetDatabase.LoadAssetAtPath<SceneAsset>(s.path).name,
                            s.enabled)).ToArray());
            ...
            public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
            {
                ...
                if (property.propertyType == SerializedPropertyType.String)
                {
                    ...
                    {
                        List<string> displayNames = SceneNames.SelectIf(Predicate, k => k.Key).ToList();
                        bool Predicate(KeyValuePair<string, bool> scene)
                        {
                            return sceneSelectorAttribute.ShowNotEnabledScenes || scene.Value;
                        }

                        EditorGUIUtils.DrawOptionsPopUpForStringProperty(position, label.text, property, displayNames);
...
//                        else
//                        {
//                            currentIndex = currentPairs.Count;
